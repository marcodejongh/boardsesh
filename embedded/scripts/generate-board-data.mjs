...
/**
 * Dynamically import the TypeScript data files by stripping TS types
 * and evaluating as JS. The generated files are mostly pure data with
 * minimal TS-only syntax.
 */
function loadDataFile(filePath) {
  let content = fs.readFileSync(filePath, 'utf-8');

  // Strip TypeScript imports
  content = content.replace(/^import\s+.*$/gm, '');

  // Strip TypeScript type annotations from exports
  // e.g., "export const FOO: Record<string, number> = {" -> "export const FOO = {"
  ...
  // Convert export const to module.exports assignments
  const exports = {};
...
/**
 * Parse the data files using regex extraction instead of eval
 * This is safer and handles the TS files without needing to strip all types
 */
