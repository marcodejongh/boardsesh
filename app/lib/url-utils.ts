...
const getApiBaseUrl = () => {
  // For server-side calls, use VERCEL_URL if available
...
const fetchLayoutBySlug = async (boardName: string, slug: string): Promise<LayoutRow | null> => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`URL-utils: Failed to fetch layout: ${baseUrl}/api/v1/${boardName}/slugs/layout/${slug} ${response.statusText}`);
    }
